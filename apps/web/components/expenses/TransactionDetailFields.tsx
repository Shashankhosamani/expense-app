...
export function TransactionDetailFields({ transaction: t }: { transaction: Transaction }) {
  ...
  return (
    <div className="flex flex-col gap-2.5">
      <span className="text-[10px] font-semibold uppercase tracking-wider text-ink-4">Straight from the message</span>
      <div className="flex flex-col">
        {rows.map(([k, v]) => (
          <div key={k} className="flex items-center gap-3.5 py-2.5 border-b border-border-3">
            <span className="w-30 text-[11px] text-ink-4">{k}</span>
