...
export default function SignInPage() {
  ...
  return (
    <div className="min-h-screen flex bg-surface">
      <div className="hidden lg:flex w-[600px] shrink-0 bg-navy p-14 flex-col justify-between">
        <Logo textClassName="text-[22px] text-surface" />
        <div className="flex flex-col gap-9">
          <h1 className="text-[42px] leading-[1.24] text-surface font-normal tracking-tight max-w-[430px] text-wrap-pretty">
          ...
          <div className="flex flex-col">
            {PIPELINE.map((step, i) => (
              <div key={step} className="flex items-baseline gap-3.5 py-2.5 border-b border-dashed border-[#1E3742]">
                <span className="w-5 text-[11px] text-brand">{i + 1}</span>
                <span className="flex-1 text-[13px] text-[#7E9CA7]">{step}</span>
              </div>
            ))}
          </div>
        </div>
        <p className="text-xs leading-5 text-[#7B96A1] max-w-[420px]">
      ...
      <div className="flex-1 flex items-center justify-center p-8 sm:p-14">
        <form onSubmit={handleSubmit} className="w-full max-w-[392px] flex flex-col gap-7">
          <div className="lg:hidden mb-2">
            <Logo />
          </div>
          <div className="flex flex-col gap-1.5">
            <h2 className="text-[32px] tracking-tight font-medium text-ink">Welcome back</h2>
          ...
          <button
            type="submit"
            disabled={loading}
            className="bg-brand hover:bg-brand-dark text-white rounded-lg px-4 py-3.5 text-[15px] font-medium cursor-pointer disabled:opacity-60"
          >
          ...
          <button
            ...
            className="flex items-center justify-center gap-2.5 bg-white hover:bg-surface-raised border border-border rounded-lg px-4 py-3.5 text-[15px] font-medium text-ink cursor-pointer disabled:opacity-60"
          >
            <GoogleIcon size={17} />
